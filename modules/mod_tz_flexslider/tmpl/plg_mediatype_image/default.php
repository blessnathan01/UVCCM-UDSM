...
//    if($params -> get('mt_show_image',1)):
            ...
            $flexThumbnail  = JURI::root().TZ_Portfolio_PlusFrontHelper::getImageURLBySize($media -> url,
                    $params -> get('mt_image_flex_thumbnail_size', 'o'));
        ?>
        <div class="tz_portfolio_plus_image">
            <a href="<?php echo $item -> link;?>">
                <img src="<?php echo $image -> url;?>" data-tpp-flex-thumb="<?php echo $flexThumbnail;
                ?>" alt="<?php echo isset($image -> caption)?$image -> caption:$item -> title;
                ?>" title="<?php echo isset($image -> caption)?$image -> caption:$item -> title;
                ?>" data-tpp-flex-thumb-alt="<?php echo isset($image -> caption)?$image -> caption:$item -> title;
                ?>" itemprop="image"/>
            </a>
        </div>
<!--    --><?php //endif;?>

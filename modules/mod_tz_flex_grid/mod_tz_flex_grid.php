...
if($list) {
	...
	switch ($gallerytype) {
		...
		case 'grid':
			$style  =   '#tz-flex-grid-'.$module -> id.' > .grid {grid-template-columns: repeat(auto-fill, minmax('.$flex_grid_width.'px, 1fr)); grid-auto-rows: minmax('.$flex_grid_height.'px, '.round($flex_grid_height*1.2).'px);}';
			$doc -> addStyleDeclaration($style);
